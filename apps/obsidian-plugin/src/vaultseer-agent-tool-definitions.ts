import type { AllowedCodexTool, CodexToolRequestClass } from "./codex-tool-dispatcher";
...
const VAULTSEER_AGENT_TOOL_DEFINITIONS: VaultseerAgentToolDefinition[] = [
  readTool(
    ...
    "Inspect current note",
    "Inspect the active note, chunks, links, tags, and related context.",
    EMPTY_INPUT_SCHEMA
  ),
  readTool(
    "inspect_index_health",
    "Inspect index health",
    "Inspect stored note, chunk, vector, source, suggestion, and queue counts.",
    EMPTY_INPUT_SCHEMA
  ),
  ...
  readTool(
    "list_vault_images",
    "List vault images",
    "List image files inside the Obsidian vault that Vaultseer may read for visual analysis.",
    LIST_VAULT_IMAGES_SCHEMA
  ),
  readTool(
    "read_vault_image",
    "Read vault image",
    "Attach an indexed vault image to the current agent turn for visual analysis.",
    READ_VAULT_IMAGE_SCHEMA
  ),
  ...
  readTool(
    ...
    "Inspect narrow quality issues such as missing tags, duplicate aliases, malformed tags, and broken links.",
    EMPTY_INPUT_SCHEMA
  ),
  ...
  commandTool(
    ...
    "Import vault text source",
    "Import one vault text/code file as a searchable source workspace.",
    IMPORT_VAULT_TEXT_SOURCE_SCHEMA
  ),
  commandTool(
  ...
  proposalTool(
    ...
    "Review current-note proposal",
    "Approve, defer, reject, or apply a staged proposal for the active note.",
    REVIEW_CURRENT_NOTE_PROPOSAL_SCHEMA
  )
];
