...
This checklist is for a small personal vault trial. It verifies that Vaultseer can index, search, review a source-note proposal, apply one approved tag update, and create one approved source note without broad write access.
...
7. Open or choose a text/code source file and run one source intake command:
...
8. Run `Vaultseer: Search stored source workspaces`.
...
9. From the source preview, review the source-note creation proposal.
...
11. Approve the source-note proposal, then press `Create note`.
