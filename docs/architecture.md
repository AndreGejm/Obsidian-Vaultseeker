...
The current apply paths are the guarded Phase 6 source-note creation flow and approved tag-update flow described below. Vaultseer can also stage a preview-only link-update proposal from the workbench, but link proposals are not applyable yet. Vaultseer still cannot rename tags, apply arbitrary frontmatter cleanup, edit aliases, copy attachments, batch apply proposals, or apply anything automatically.
...
The view refreshes when Obsidian opens another file and after Vaultseer rebuilds or clears the index. It opens notes through Obsidian when the operator clicks a related note, resolved link, or suggested link target. It can stage suggested tags and suggested unresolved-link fixes as guarded write review proposals, but those actions only store suggestion and operation records. Tag proposals are editable only later, from the guarded write review queue, after explicit approval and a current-file hash check. Link proposals are currently preview-only in the queue and cannot be applied yet.
...
Current limitation: the workbench is still mostly a read-only mirror inspector. It has guarded-action bridges for staging suggested tags and link fixes into the write review queue. It does not apply tag updates directly, apply link updates, expose suggestion decisions inline, show semantic current-note results, or provide a full gardener queue.
...
Core now has existing-note tag and link update operations. `packages/core/src/writes/guarded-write.ts` exposes `planNoteTagUpdateOperation`, which accepts a target note path, current Markdown content, proposed tags, suggestion IDs, and a timestamp. It normalizes frontmatter tags, preserves unrelated frontmatter fields, produces modified Markdown content, stores the expected current content hash, and creates a full-file preview diff. `packages/core/src/suggestions/suggestion-records.ts` converts workbench tag suggestions into stable `note_tag` suggestion records with evidence for linked-note tags, backlink tags, co-tag statistics, and tag frequency. This operation is reviewable through the shared guarded-write queue and applyable only after approval and a fresh precondition check.

`planNoteLinkUpdateOperation` accepts a target note path, current Markdown content, unresolved-link replacements, suggestion IDs, and a timestamp. It rewrites exact unresolved wiki-link strings into explicit Obsidian links that preserve the visible display text, stores the expected current content hash, and creates a full-file preview diff. `createNoteLinkSuggestionRecords` stores `note_link` suggestion records with unresolved-link, alias/title-match, and token-overlap evidence. The workbench can stage these records through `apps/obsidian-plugin/src/link-write-proposal-controller.ts`, but `update_note_links` remains preview-only: the queue can approve/defer/reject the operation, but `canApply` is false and the Obsidian write port rejects accidental link-update apply calls.
